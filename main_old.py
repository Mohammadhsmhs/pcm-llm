...
from config import (
    ...
    NUM_SAMPLES_TO_RUN, OPENAI_MODEL,
...
def run_benchmark_for_task(task_name: str):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(intermediate_file, 'r', newline='', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                if evaluated_count == 0:  # Only log memory for first sample
                    log_memory_usage("during evaluation", run_info_logger)
                ...
                run_info_logger.log_task_completion(task_data)
                ...
                if evaluated_count % 3 == 0:  # More frequent progress updates
                    ...
                    run_info_logger.log_memory_usage(current_memory)

        # Cleanup
        del target_llm
        clear_memory()
        log_memory_usage("after evaluation phase", run_info_logger)
        ...
        final_stats = {
            ...
            "task_name": task_name,
            "compression_methods": list(COMPRESSION_METHODS_TO_RUN),
            "final_memory_usage": psutil.virtual_memory().used / (1024 * 1024)
        }
        run_info_logger.finalize_run(final_stats)

        print(f"✅ {task_name.upper()} optimized benchmark completed!")
        print(f"   📊 Results saved to: {base_logger.log_dir}")
        print(f"   📈 Analysis report: {analysis_file}")
        print(f"   📋 Run info: {run_info_logger.run_info_file}")
        print(f"   📝 Task log: {run_info_logger.task_log_file}")
        print(f"   🔍 Real-time log: {run_info_logger.real_time_log_file}")
...
def run_optimized_multi_task_benchmark(tasks_to_run=None):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        clear_memory()
        log_memory_usage("after compression phase", run_info_logger)
        ...
        # Update run info with configuration
        run_config = {
            "task_names": list(tasks_to_run),
            "compression_methods": list(COMPRESSION_METHODS_TO_RUN),
            "llm_provider": DEFAULT_LLM_PROVIDER,
            "llm_model": get_model_name(DEFAULT_LLM_PROVIDER),
            "num_samples_per_task": NUM_SAMPLES_TO_RUN,
            "target_compression_ratio": DEFAULT_TARGET_RATIO,
            "unlimited_mode": UNLIMITED_MODE,
            "total_samples": len(tasks_to_run) * NUM_SAMPLES_TO_RUN
        }
        run_info_logger.update_run_config(run_config)
        ...
        for task_name in tasks_to_run:
            ...
            for sample_data in master_data[task_name]:
                ...
                if evaluated_count == 0:  # Only log memory for first sample per task
                    log_memory_usage(f"during {task_name} evaluation", run_info_logger)
        ...
        final_stats = {
            ...
            "tasks_completed": list(all_task_results.keys()),
            "compression_methods": list(COMPRESSION_METHODS_TO_RUN),
            "final_memory_usage": psutil.virtual_memory().used / (1024 * 1024),
            "samples_per_task": all_task_results
        }
        run_info_logger.finalize_run(final_stats)
