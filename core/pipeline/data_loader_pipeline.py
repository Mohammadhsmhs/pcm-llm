...
class DataLoaderPipeline:
    ...
    def __init__(
        self, tasks: List[str], num_samples: int, run_info_logger: RunInfoLogger
    ):
        self.tasks = tasks
    ...
    def run(self) -> Dict[str, Dict[str, Any]]:
        ...
        for task_name in self.tasks:
            ...
            if samples:
                print(
                    f"DEBUG: DataLoader output for '{task_name}' sample 0: {str(samples[0])[:200]}..."
                )

        log_memory_usage("after loading all datasets", self.run_info_logger)
        return all_samples_data

    def _load_and_cache_samples(
        self, task_name: str, num_samples: int
    ) -> List[Dict[str, Any]]:
        """
        ...
        if cached_samples:
            print(
                f"📖 Samples loaded from cache for '{task_name}': {len(cached_samples)} samples"
            )
        ...
        dataset = load_benchmark_dataset(
            task_name, task_config.dataset, task_config.config, num_samples
        )
