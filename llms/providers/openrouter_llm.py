            ...
            try:
                ...
                if "rate limit" in error_str or "429" in error_str:
                    if attempt < self.config.max_retries - 1:
                        wait_time = 60
                        print(f"⏱️  Rate limit hit. Waiting {wait_time} seconds before retry {attempt + 1}/{self.config.max_retries}...")
                        time.sleep(wait_time)
                        continue
                    else:
                        print(f"❌ Rate limit persisted after {self.config.max_retries} attempts")
