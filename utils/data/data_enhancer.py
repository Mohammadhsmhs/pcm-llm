class DataEnhancer:
    ...
    def enhance_result_data(result_data):
        ...
        if baseline_score > 0:
            enhanced_data['score_preservation'] = (compressed_score / baseline_score) * 100
        else:
            enhanced_data['score_preservation'] = 0.0 if compressed_score == 0 else 100.0

        # Quality Degradation
        enhanced_data['quality_degradation'] = baseline_score - compressed_score
        enhanced_data['quality_degradation_percent'] = enhanced_data['score_preservation'] - 100
        ...
        if baseline_latency > 0:
            enhanced_data['latency_overhead_percent'] = (latency_overhead / baseline_latency) * 100
        else:
        ...
        if target_ratio > 0:
            ...
            efficiency = (1 - actual_ratio) / (1 - target_ratio) if target_ratio < 1 else 1.0
            enhanced_data['compression_efficiency'] = min(efficiency, 1.0) * 100  # Cap at 100%
        else:
