    ...
    def write_csv(self, sample_data, compression_methods):
        ...
        for method in sorted_methods:
            fieldnames.extend(
                [
                    ...
                    f"{method}_latency_overhead_seconds",
                    f"{method}_latency_overhead_percent",
                    f"{method}_quality_degradation",
                    f"{method}_quality_degradation_percent",
                ]
            )
        ...
        for sample_id, sample in sample_data.items():
            record = {
                ...
                "baseline_extracted_answer": sample.get(
                    "baseline_extracted_answer", ""
                ),
            ...
            for method in sorted_methods:
                ...
                if method_data:
                    record.update(
                        {
                            f"{method}_compressed_prompt": method_data.get(
                                "compressed_prompt"
                            ),
                            f"{method}_compressed_output": method_data.get(
                                "compressed_output"
                            ),
                            f"{method}_compressed_extracted_answer": method_data.get(
                                "compressed_extracted_answer"
                            ),
                            f"{method}_compressed_score": method_data.get(
                                "compressed_score"
                            ),
                            f"{method}_compressed_latency": method_data.get(
                                "compressed_latency"
                            ),
                            f"{method}_answers_match": method_data.get("answers_match"),
                            f"{method}_target_ratio": method_data.get(
                                "target_compression_ratio"
                            ),
                            f"{method}_actual_ratio": method_data.get(
                                "actual_compression_ratio"
                            ),
                            f"{method}_compression_efficiency": method_data.get(
                                "compression_efficiency"
                            ),
                            f"{method}_tokens_saved": method_data.get("tokens_saved"),
                            f"{method}_score_preservation": method_data.get(
                                "score_preservation"
                            ),
                            f"{method}_latency_overhead": method_data.get(
                                "latency_overhead"
                            ),
                            f"{method}_latency_overhead_seconds": method_data.get(
                                "latency_overhead_seconds"
                            ),
                            f"{method}_latency_overhead_percent": method_data.get(
                                "latency_overhead_percent"
                            ),
                            f"{method}_quality_degradation": method_data.get(
                                "quality_degradation"
                            ),
                            f"{method}_quality_degradation_percent": method_data.get(
